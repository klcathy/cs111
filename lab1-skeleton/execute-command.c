...
char** split_string(char* str)
{
	int num = 0; // Number of strings split
	char* s;
	char** result = (char**)checked_malloc(sizeof(char*));
	s = strtok (str," "); //Tokenizes each string component
	while (s != NULL)
	{
		result = checked_realloc(result, (num+2)*sizeof(char*)); // 1 for string, 1 for NULL
		result[num] = s;
		num++; 
		result[num] = NULL; 
		s = strtok (NULL, " ");
...
void executingSimple(command_t c)
{
	...
	else
	{
		...
		char **split = split_string(*c->u.word);
